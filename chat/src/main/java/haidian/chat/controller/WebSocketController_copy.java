...
//@Component
public class WebSocketController_copy {
    ...
    private static ConcurrentHashMap<String, WebSocketController_copy> websocketList=new ConcurrentHashMap<>();
    //接收id
...
//                    object.put("fromUserId",this.userId);
                    ...
                    if(StringUtils.isNotBlank(toUserId)&&StringUtils.isNotBlank(contentText)){
                        WebSocketController_copy toSocket=websocketList.get(toUserId);
                        //需要进行转换，userId
                        if(toSocket!=null){
                        ...
                        if(toSocket==null){
                            sendMessage("对面离线");
                            WebSocketController_copy fromSocket=websocketList.get(fromUserId);
    ...
    public static synchronized void addOnlineCount() {
        WebSocketController_copy.onlineCount++;
    }
    ...
    public static void setRedisUtil(RedisUtil redisUtil) {
        WebSocketController_copy.redisUtil = redisUtil;
    }
    public static void setStringRedisTemplate(StringRedisTemplate stringRedisTemplate) {
        WebSocketController_copy.stringRedisTemplate = stringRedisTemplate;
    }
}
